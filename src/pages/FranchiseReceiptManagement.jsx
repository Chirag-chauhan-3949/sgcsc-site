// src/pages/FranchiseReceiptManagement.jsx
...
export default function FranchiseReceiptManagement() {
  ...
  const fetchReceipts = useCallback(async () => {
    try {
      ...
      const response = await API.get(`/franchise/receipts?${params}`);
  ...
  const handleDelete = async (receiptId) => {
    ...
    try {
      await API.delete(`/franchise/receipts/${receiptId}`);
  ...
  const handleUpdate = async () => {
    setSaving(true);
    try {
      await API.put(`/franchise/receipts/${selectedReceipt._id}`, editForm);
    ...
    <FranchiseLayout>
      <div className="container-fluid p-4">
        <div className="d-flex justify-content-between align-items-center mb-4">
          ...
          <button className="btn btn-primary" onClick={fetchReceipts}>Refresh</button>
        </div>
                  ...
                  <tbody>
                    {receipts.map((receipt) => (
                      <tr key={receipt._id}>
                        ...
                        <td>
                          <button className="btn btn-sm btn-info me-2" onClick={() => printReceipt(receipt)} title="Print">🖨️</button>
                          <button className="btn btn-sm btn-warning me-2" onClick={() => handleEdit(receipt)} title="Edit">✏️</button>
                          <button className="btn btn-sm btn-danger" onClick={() => handleDelete(receipt._id)} title="Delete">🗑️</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <div className="mt-4">
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setEditForm({ ...editForm, monthlyPayments: [...editForm.monthlyPayments, newPayment] });
                        }}
                      >
                        + Add Month
                      </button>
                    ...
                    <div className="table-responsive">
                      <table className="table table-sm">
                        <thead>
                          <tr>
                            <th>Month</th><th>Date</th><th>Paid</th><th>Due</th><th>Status</th><th>Action</th>
                          </tr>
                        </thead>
                        <tbody>
                          {editForm.monthlyPayments?.map((payment, index) => (
                            <tr key={index}>
                              <td>
                                <input
                                  type="text"
                                  ...
                                  value={payment.month}
                                  onChange={(e) => {
                                    const updated = [...editForm.monthlyPayments];
                                    updated[index] = { ...payment, month: e.target.value };
                                    setEditForm({ ...editForm, monthlyPayments: updated });
                                  }}
                                />
                              </td>
                              <td>
                                <input
                                  ...
                                  onChange={(e) => {
                                    const updated = [...editForm.monthlyPayments];
                                    updated[index] = { ...payment, date: e.target.value };
                                    setEditForm({ ...editForm, monthlyPayments: updated });
                                  }}
                                />
                              </td>
                              <td>
                                <input
                                  ...
                                  onChange={(e) => {
                                    const updated = [...editForm.monthlyPayments];
                                    updated[index] = { ...payment, paid: parseFloat(e.target.value) || 0 };
                                    setEditForm({ ...editForm, monthlyPayments: updated });
                                  }}
                                />
                              </td>
                              <td>
                                <input
                                  ...
                                  onChange={(e) => {
                                    const updated = [...editForm.monthlyPayments];
                                    updated[index] = { ...payment, due: parseFloat(e.target.value) || 0 };
                                    setEditForm({ ...editForm, monthlyPayments: updated });
                                  }}
                                />
                              </td>
                              <td>
                                <select
                                  ...
                                  onChange={(e) => {
                                    const updated = [...editForm.monthlyPayments];
                                    updated[index] = { ...payment, status: e.target.value };
                                    setEditForm({ ...editForm, monthlyPayments: updated });
                                  }}
                                >
                              ...
                              <td>
                                <button
                                  ...
                                  onClick={() => {
                                    const updated = editForm.monthlyPayments.filter((_, i) => i !== index);
                                    setEditForm({ ...editForm, monthlyPayments: updated });
                                  }}
                                >
                                  Remove
